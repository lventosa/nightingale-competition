...
y.hat1 <- predict(reg2, data=test_data)
...
training1 <- data.frame(y=train_data$total_deaths, lag1=train_data$total_deaths) 
#model1 <- lm(y ~ lag1, data=training1)
y.hat2 <- predict(reg4, data=test_data)
error2 <- abs(y.hat2 - test_data$total_deaths)
rmse2 <- sqrt((sum(error2)**2)/nrow(test_data))

head(y.hat2)

#Add predictions:
mydata <- cbind(data, y.hat2)

# Regression line + confidence intervals:
mydata$group=row.names(mydata)
colnames(mydata)=month.abb
p <- ggplot(mydata, aes(group, data$avg_size_army)) +
  geom_point() +
  stat_smooth(method = lm)
p
```

We can appreciate that the results of both predictions are not accurate. This fact is produced due to the size of the dataset and the number of variables we are working with. It is known that, in order to have much more accurate and reliable results, more variable need to be taken in account, however this informationwas not given in the dataset. It is also important to highlight the time in which we data was collected (1854-1856) to better understand the lack of information compiled by Nightingale and the tools he used to process this data.
