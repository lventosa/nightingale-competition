...
Moreover, the data that is presented in the dataset we are going to analyze it refers to Florence Nightingale's count of injuries in the Crimean War. This data was going to be later used for her rose plots.


Per esborrar?
(On hauriem de citar? Aquí o creem bibliografia abaix?
...
(No he posat exactament el tipus de data que conté perquè he pensat que ja ho descriurem abaix))
...
In Nighingale's dataset, the following variables come as is:
- Month: Character variable stating the month and year (period).
- Average size of army: Numeric variable that informs the reader about the average size of the army in such period of time.
...
Per esborrar i tria títol o fem pluja d'idees:
...
In order to have a better understanding of the data presented, we created a new variable which contains the sum of the different deaths causes (zymotic, injuries and other) combined. Each entry of this variable will be seggregated by period/month of the year. The name of this new variable is "deaths".

In addition, we created another new variable that aims to explain the cumulative deaths over time. Thus, using the new variable of deaths created just before, we create a new one in order to explain how this aggregated deaths perform over time. The name of this variable is "cum_deaths". 
...
```{r}
#Checking outliers:
par(mfrow=c(1,3)) #Amb això es veuen tots tres boxplots alhora ueeee. Yeeeeeeeeeey!!! :D (L)
...
The aim on this section is to show the different correlations between variables through regressions.
