...
In this section, we are going to explore the dataset in a visual way to see the impacts some variables had on some others. EXPLICACIO + FOTO DELS ROSE PLOTS QUE ELLA VA FER ORIGINALMENT.

The causes of death can be represented in barplots such as the following ones:
```{r}
...
An alternative approach is to represent the above three barplots in one signle graph. The following plot is an interactive graph that shows the evolution of the three series (zymotic diseases, injuries and other) depicted all together:
...
data3=melt(data_1year, id="month")
...
```{r}
#Bar graph:
ggplot(data=data4,aes(x=value, y=month, fill=variable))+
        geom_bar(stat="identity")+
        scale_fill_brewer(palette="RdBu")+xlab("")+ylab("")

ggplot(data=data4.1,aes(x=value, y=month, fill=variable))+
        geom_bar(stat="identity")+
        scale_fill_brewer(palette="RdBu")+xlab("")+ylab("")

ggplot(data=data4.2,aes(x=value, y=month, fill=variable))+
        geom_bar(stat="identity")+
        scale_fill_brewer(palette="RdBu")+xlab("")+ylab("")

#yayyyy! és un avenç jajajaj però com és que queden les dates així desordenades? Quin lio treballar amb dates tan antigues que R no reconeix com a dates jajajaaj
```
```{r}
#Coordenades polar graph:

 df1  <-  melt(data, id="month")
...
ggplot(data=data4,aes(x=month,y=value,fill=variable))+
        geom_bar(stat="identity")+
        coord_polar()+
...
Besides, it is very important to take into account when the data was collected, years 1854-1856, to better understand the lack of information collected by Nightingale. Due to this fact, it is difficult to identify clear patterns in the overall data. As a matter of fact, we can appreciate sudden increases in the data on average size army although deaths are high, one could assume that larger deaths caused the size of the army to move in two opposite directions. On the one hand, more deaths caused the size of the army to decrease. On the other hand, the more deaths the more new soldiers that were recruited to join the army, making the average size of the army to increase. Apparently, this last effect was stronger than the other. Nevertheless, we do not have data to sustain this theory and to calculate the exact size of the impact these two effects had.

Moreover, due to the few data points collected and the lack of other important variables, most of the regessions performed are very poor, showing no relevant significance between the variables analyzed. The small size of the dataset also poses a problem when trying predict future values using some of the best regression models as the accuracy obtained is extremely low judging by the resulting root mean squared error values.
