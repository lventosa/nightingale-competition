...
- The majority of deaths by injury are compressed by 0 and 130 approximately. Additionally, there are some cases up to 300, however excepcional.
- The majority of deaths by zymotic disease are compressed between 50 and 800 approximately. However, we can see a long tail of cases up to 1700, with some additional outliers of 2000 and 2800.
...
To sum up, it seems for now that the majority of deaths were caused by zymotic diseases.
...
We are going to explore the dataset in a visual way to see the different impacts that some variables had on others:
...
data <- data  %>%
  ...
  mutate(percentage = n / sum(n)) #No em deixa córrer aquesta funció:(
